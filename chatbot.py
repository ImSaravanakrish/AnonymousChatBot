...
botton=[[
      ...
      InlineKeyboardButton("Tamil Chat Group🇮🇳", url=f"https://TamilChatGroup")
      ]]
...
def welcome(message):
    if check_user(user_id=message.from_user.id)[0]:
        ...
        bot.send_message(message.from_user.id, f"*Welcome to Join {BOT_NAME}🙊*\n\n_Hope you get a friend or a mate_\n\n*NOTE:*\nJOIN\n[👥 Tamil Chat ɢʀᴏᴜᴘ](t.me/{GROUP}) | [Bot ᴄʜᴀɴɴᴇʟ 📣](t.me/{CHANNEL}) | [🧑‍💻 developer](t.me/{OWNER})",parse_mode="markdown",disable_web_page_preview=True, reply_markup=mark)
        bot.register_next_step_handler(message, search_prof)
    else:
        bot.send_message(message.from_user.id, "_👋Hello New Users,\n\n To Continue the chat 🫣 Filling The Following Bio data!_",parse_mode="markdown")
